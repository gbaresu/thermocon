...
c                                                ** this level
   ...
   70    CONTINUE
         IF( PRNT(2) ) WRITE(*,'(E10.4,I7,1P,7E12.3,E14.3)') UTAU( LU ),
...
c                      ** Find (real) eigenvalues and eigenvectors
     ...
     &             EVECCD, EVALD )
      ...
      IF( IER.GT.0 ) THEN
         ...
         WRITE( 10, '(//,A,I4,A)' ) ' ASYMTX--eigenvalue no. ',
...
c     ..
     ...
     &   '(at user polar angles)  ********'
      ...
      WRITE( 10,'(/,A,/,A)') '   Optical   Polar Angle Cosines',
     &                      '     Depth'
      ...
      DO 20 NP = 1, NPASS
         ...
         WRITE( 10,'(/,10X,8F14.5)') ( UMU(IU), IU = IUMIN, IUMAX )

         DO 10 LU = 1, NTAU
            WRITE( 10, '(0P,F10.4,1P,8E14.4)' ) UTAU( LU ),
...
c     ..
     ...
     &       '     No. computational layers =', NLYR
      ...
      IF( IBCND.NE.1 ) WRITE( *, '(I4,A,10F10.4,/,(26X,10(E9.3,1X)))' )
...
c                                       ** two WRITEs above)
         IF( PLANK )
     &WRITE(*,'(I3,2(1X,E10.4),2(1X,E12.5),4(2X,E10.4),F9.4,1X,F14.3)')
     &             LC, DTAUC( LC ), TAUC( LC ), SSALB( LC ), FLYR( LC ),
     &             DTAUCP( LC ), TAUCPR( LC ), OPRIM( LC ), PMOM(1,LC),
     &             TEMPER( LC-1 )

         IF( .NOT.PLANK )
     &       WRITE(*,'(I3,2(1X,F10.4),3(1X,F12.5),F9.4)')
     &             LC, DTAUC( LC ), TAUC( LC ), SSALB( LC ), FLYR( LC ),
     &             DTAUCP( LC ), TAUCPR( LC ), OPRIM( LC ), PMOM( 1,LC )
   10 CONTINUE

      IF( PLANK ) WRITE( 10, '(85X,F14.3)' ) TEMPER( NLYR )


      IF( PRTMOM .AND. YESSCT.GT.0.0 ) THEN

         WRITE( 10, '(/,A)' ) ' Layer   Phase Function Moments'

         DO 20 LC = 1, NLYR

            IF( SSALB( LC ).GT.0.0 )
     &          WRITE( 10, '(I6,10F11.6,/,(6X,10F11.6))' )
     ...
     &   '    Depth   Cosine'
      ...
      DO 30 LU = 1, NTAU
         ...
         DO 20 NP = 1, NPASS
            ...
            WRITE( 10, '(/,18X,10F11.2)' ) ( PHI(J), J = JMIN, JMAX )

            IF( NP.EQ.1 ) WRITE( 10, '(F10.4,F8.4,1P,10E11.3)' )
     &             UTAU(LU), UMU(1), (UU(1, LU, J), J = JMIN, JMAX)
            IF( NP.GT.1 ) WRITE( 10, '(10X,F8.4,1P,10E11.3)' )
     &                       UMU(1), (UU(1, LU, J), J = JMIN, JMAX)

            DO 10 IU = 2, NUMU
               WRITE( 10, '(10X,F8.4,1P,10E11.3)' ) 
...
c     ..
     ...
     &  ' Beam Zen Ang   cos(Beam Zen Ang)      Albedo   Transmissivity'
      ...
      DO 10 IU = 1, NUMU
         WRITE( 10, '(0P,F13.4,F20.6,F12.5,1P,E17.4)' )
